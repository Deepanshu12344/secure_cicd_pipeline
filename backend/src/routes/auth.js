...
router.post('/register-faculty', asyncHandler(async (req, res) => {
  ...
  const openRegistration = String(process.env.FACULTY_OPEN_REGISTRATION || 'true').toLowerCase() !== 'false';
  if (!openRegistration) {
    return res.status(403).json({
      success: false,
      error: 'Faculty self-registration is disabled'
    });
  }
  ...
  const allowedDomains = String(process.env.FACULTY_ALLOWED_EMAIL_DOMAINS || '')
    .split(',')
  ...
  if (allowedDomains.length > 0 && !allowedDomains.includes(domain)) {
    return res.status(403).json({
      success: false,
      error: 'Email domain is not approved for faculty registration'
    });
  }
...
router.post('/register-student', asyncHandler(async (req, res) => {
  ...
  const { student_id, name, github_repo, email, password } = req.body;

  if (!student_id || !name || !github_repo || !email) {
    return res.status(400).json({
      success: false,
      error: 'student_id, name, github_repo, and email are required'
    });
  }
  ...
  await Student.findOneAndUpdate(
    { studentId: normalizedStudentId },
    {
      studentId: normalizedStudentId,
      name: String(name).trim(),
      githubRepo: String(github_repo).trim(),
      email: normalizedEmail
    },
    { upsert: true, new: true, setDefaultsOnInsert: true }
  );

  const initialPassword = String(password || normalizedStudentId);
  const newUser = await User.create({
    email: normalizedEmail,
    name: String(name).trim(),
    role: 'student',

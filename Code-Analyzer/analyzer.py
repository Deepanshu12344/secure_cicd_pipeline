...
class RepositoryAnalyzer:
    ...
    def analyze_file_heuristically(self, file_path, file_content, chunk_info=None):
        """Fallback analysis used when API analysis fails."""
        ...
        complexity_hits = sum(1 for line in lines if any(k in line for k in [' if ', ' for ', ' while ', ' switch ', ' case ']))

        documentation_score = max(20, min(95, int((comment_lines / total_lines) * 140)))
        complexity_score = max(30, min(95, 85 - min(50, int((complexity_hits / total_lines) * 180))))
        maintainability_score = max(25, min(95, 90 - min(50, int((long_lines / total_lines) * 220))))
        efficiency_score = max(35, min(95, 88 - min(40, int((complexity_hits / total_lines) * 140))))
        accuracy_score = max(40, min(95, 90 - min(35, todo_count * 4)))
        overall_score = int(round((accuracy_score + complexity_score + efficiency_score + maintainability_score + documentation_score) / 5))
        ...
        if long_lines > 0:
            issues.append({
                "issue": "Very long lines reduce readability",
                "location": "Multiple lines",
                "code": "Lines over 120 characters detected",
                "severity": "Medium",
                "reason": "Long lines are harder to review and maintain.",
                "category": "CodeQuality"
            })
            suggestions.append({
                "suggestion": "Wrap long lines and split complex expressions",
                "location": "Multiple lines",
                "current": "Long inline statements",
                "recommended": "Extract sub-expressions and keep lines short",
                "benefit": "Improves maintainability and review speed.",
                "category": "CodeQuality"
            })
            quality_issues.append({
                "type": "Readability",
                "location": "Multiple lines",
                "description": "Lines exceed 120 characters.",
                "priority": "Medium",
                "category": "CodeQuality"
            })
        ...
        if todo_count > 0:
            issues.append({
                "issue": "TODO/FIXME markers found",
                "location": "Multiple lines",
                "code": "TODO/FIXME comments present",
                "severity": "Low",
                "reason": "Incomplete sections may hide issues.",
                "category": "CodeQuality"
            })

        if documentation_score < 55:
            quality_issues.append({
                "type": "Documentation",
                "location": "File level",
                "description": "Low comment density detected.",
                "priority": "Medium",
                "category": "CodeQuality"
            })
        ...
        if not issues:
            issues.append({
                "issue": "No major static issues detected in heuristic mode",
                "location": "File level",
                "code": "",
                "severity": "Low",
                "reason": "Heuristic checks found no major structural issues.",
                "category": "CodeQuality"
            })

        analysis = {
            "filePurpose": f"Heuristic analysis of {file_path}.",
            "accuracy": {"score": accuracy_score, "explanation": "Estimated from TODO/FIXME indicators.", "details": [f"TODO/FIXME count: {todo_count}"]},
            "complexity": {"score": complexity_score, "explanation": "Estimated from branching density.", "details": [f"Control-flow indicators: {complexity_hits}"]},
            "efficiency": {"score": efficiency_score, "explanation": "Estimated from structural complexity.", "details": [f"Long lines: {long_lines}"]},
            "maintainability": {"score": maintainability_score, "explanation": "Estimated from readability patterns.", "details": [f"Total lines: {total_lines}"]},
            "documentation": {"score": documentation_score, "explanation": "Estimated from comment ratio.", "details": [f"Comment lines: {comment_lines}/{total_lines}"]},
            "overallScore": overall_score,
            "strengths": [{"point": "Fallback scan completed", "location": "File level", "impact": "Provides baseline quality signals."}],
            "weaknesses": issues,
            "suggestions": suggestions,
            "codeQualityIssues": quality_issues
        }
    ...
    def analyze_file_with_claude(self, file_path, file_content, chunk_info=None, repo_context=None):
        ...
        if not self.api_key:
            return self.analyze_file_heuristically(file_path, file_content, chunk_info)
